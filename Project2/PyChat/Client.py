...
class ChatClient:
    def __init__(self, host, port):
        ...
        self.send_semaphore = threading.Semaphore(2) # Allows up to 2 simultaneous sends without releasing
    ...
    def receive_messages(self):
        while self.running:
            try:
                ...
                if not message: # If no data is received, the connection is likely closed
        ...
        while self.running:
            try:
                ...
                if message.lower() == '/exit': # If the user types '/exit', break the loop
                    break
                self.send_semaphore.acquire() # Acquire a semaphore to send a message
                ...
                self.send_semaphore.release() # Release the semaphore
...
if __name__ == "__main__":
    ...
    client = ChatClient(host, port) # Create a ChatClient instance
